...
- **伪类选择器**（Pseudo-ELement Selectors）
  ...
  - `E:nth-of-last-type(n)`：匹配父元素的倒数第n个子元素E
  - `E:empty`：匹配没有任何子元素（包括text节点）的元素E
  - `E:checked`：匹配用户界面上处于选中状态的元素E。(用于input的type为radio与checkbox时)
...
- `column-count`：设置或检索对象的列数
  - **ps**：用于做图片瀑布流不合适，因为它在进行图片的排版时会让最高的”图片柱“成为多数柱，次高的”图片柱“成为次多数柱
...
- **overflow**：定义了元素处理溢出内容的方式
  ...
  - **ps**：overflow-x：scroll可以用来实现横向滚动的图片流，例如实现相关人物图片的展示流
- **resize**：设置或检索对象的区域是否允许用户缩放，调节元素尺寸大小
  ...
  - **ps**：一般不推荐给用户使用，因为有可能会使得使用resize的元素旁边的结构产生重排和重绘
...
- **align-content**： 当伸缩容器的交叉轴还有多余空间时，本属性可以用来调准“伸缩行”整行在伸缩容器里的对齐方式，这与调准伸缩项目在主轴上对齐方式的`justify-content`属性类似。请注意本属性在只有一行的伸缩容器上没有效果（适用于：多行的弹性盒模型容器）
  ...
  - **ps**：元素设置宽高后stretch失效
- **order**：设置或检索弹性盒模型对象的子元素出現的顺序（适用于：flex子项和flex容器中的绝对定位子元素）
  - 默认值为0
...
- **perspective**：指定观察者与 z=0 平面的距离
  ...
  - `none`：默认值。不指定透视
  - **ps**：一般设置在绝对父元素上，设置perspective的同时建议也设置transform-style，以保证子元素的各种transform操作能以3d形式体现
...
- **ps**：元素设置perspective或transform-style非默认值后，该元素就成为其子元素的定位元素，无论自身有无非static的定位值
...
- 物理像素 = 设备像素：用设备的dpi或ppi来表示，一屏幕所容纳像素即 分辨率宽 * 分辨率高 * ppi

- 设备像素比dpr（Device Pixel Ration）= 物理像素 / 逻辑像素
...
- **@import**：指定导入的外部样式表及目标媒体
  ...
  - `<media_query_list>`：指定媒体类型（media types）和查询条件（media features）
...
- 引用方法有：
  ...
  - @import引用
  - @media引用
- Media Query 可以看成是 Media Type（判断条件）+ CSS（符合条件的样式规则）
- 操作符：
  - `and`：合并多个媒体属性或合并媒体属性与媒体类型。一个基本的媒体查询，即一个媒体属性与默认指定的screen媒体类型
  - `,`：指定备用功能。没有or关键词可用于指定备用的媒体功能。相反，可以将备用功能以逗号分割列表的形式列出
  - `not`：指定否定条件。在媒体声明中添加关键字not，不能在单个条件前使用not。该关键字必须位于声明的开头，而且它会否定整个声明
  - `only`：向早期浏览器隐藏媒体查询。类似于not，该关键字必须位于声明的开头
- 易混淆的宽高度：
  - `device-width、device-height`：设备的宽度
  - `width、height`：documentElement.clientWidth/Height即viewport的值。渲染宽度/高度/视口高度
- 单位值：
  - `rem`：CSS3新增的一个相对单位（root element）相对的只是HTML根元素
